...
from portfolio_core import *  # noqa: F403
...
def update_gsheets_worksheet(conn, worksheet: str, data: pd.DataFrame) -> None:
    """Replace worksheet contents in Firestore."""
...
def persist_stock_purchases(
    ...
    update_gsheets_worksheet(conn, "StockPurchases", updated)
...
def load_master_data_from_gsheets(conn=None) -> pd.DataFrame:
    """Load Portfolios from Firestore."""
    from api.sheets import read_portfolios

    return read_portfolios(use_cache=False)
...
if authentication_status is False:
    ...
    from api.connection import FirestoreConnection

    conn = FirestoreConnection()

    # Load data from Firestore into session state (once per session)
    if "master_data" not in st.session_state:
        try:
            st.session_state.master_data = load_master_data_from_gsheets(conn)
                            ...
                            if ticker in existing_tickers:
                                for i, stock in enumerate(st.session_state.stocks):
                                    if stock['name'] == ticker:
                                        ...
                                        break
                            elif ticker in KIDS_ETF_TICKERS:
